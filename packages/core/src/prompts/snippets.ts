...
- **File Editing Collisions:** Do NOT make multiple calls to the ${formatToolName(EDIT_TOOL_NAME)} tool for the SAME file in a single turn. ${
    options.hasHashline
      ? `To make multiple edits to the same file in a single turn, you MUST use the \`edits\` array parameter of the \`${EDIT_TOOL_NAME}\` tool to batch them atomically.`
      : 'To make multiple edits to the same file, you MUST perform them sequentially across multiple conversational turns to prevent race conditions and ensure the file state is accurate before each edit.'
  }
