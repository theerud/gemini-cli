...
- **Planning Tool Usage:** Only use the \`present_plan\` tool when you have a complete, multi-step implementation strategy to propose. Invoking this tool will transition the session into a read-only Planning Mode for user review. For simple answers or confirmations, use normal chat.
...
function workflowStepPlan(options: PrimaryWorkflowsOptions): string {
  let base = `2. **Plan:** Build a coherent and grounded (based on the understanding in step 1) plan for how you intend to resolve the user's task. If the user's request implies a change but does not explicitly state it, **YOU MUST ASK** for confirmation before modifying code.`;

  if (options.enableCodebaseInvestigator) {
    base += ` If 'codebase_investigator' was used, do not ignore the output of the agent, you must use it as the foundation of your plan.`;
  }

  if (options.enableWriteTodosTool) {
    base += ` For complex tasks, break them down into smaller, manageable subtasks and use the \`${WRITE_TODOS_TOOL_NAME}\` tool to track your progress.`;
  }

  base += ` Share an extremely concise yet clear plan with the user if it would help the user understand your thought process. As part of the plan, you should use an iterative development process that includes writing unit tests to verify your changes. Use output logs or debug statements as part of this process to arrive at a solution.`;
